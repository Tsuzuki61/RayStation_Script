...
class MessageBox(Window):
	def __init__(self,caption,ButtonType,*text_str):
		...
		for text in text_str:
			tmpText=TextBlock()
			tmpText.Text = text
			tmpText.TextWrapping = TextWrapping.Wrap
			tmpText.Margin = Thickness(5)
			tmpText.TextAlignment = TextAlignment.Center
			self.SPanel.Children.Add(tmpText)
		if ButtonType == 'YesNo':
			Ybtn = Button()
			Ybtn.Content = "Yes"
			Ybtn.Width = 50
			Ybtn.Margin = Thickness(5)
			Ybtn.Click += self.Yes_Clicked
			self.ButtomSPanel.Children.Add(Ybtn)
			Nbtn = Button()
			Nbtn.Content = "No"
			Nbtn.Width = 50
			Nbtn.Margin = Thickness(5)
			Nbtn.Click += self.No_Clicked
			self.ButtomSPanel.Children.Add(Nbtn)
		elif ButtonType == 'OKCancel':
			OKbtn = Button()
			OKbtn.Content = "OK"
			OKbtn.Width = 50
			OKbtn.Margin = Thickness(5)
			OKbtn.Click += self.OK_Clicked
			self.ButtomSPanel.Children.Add(OKbtn)
			Canbtn = Button()
			Canbtn.Content = "Cancel"
			Canbtn.Width = 50
			Canbtn.Margin = Thickness(5)
			Canbtn.Click += self.No_Clicked
			self.ButtomSPanel.Children.Add(Canbtn)
		elif ButtonType == 'OKOnly':
			OKbtn = Button()
			OKbtn.Content = "OK"
			OKbtn.Width = 50
			OKbtn.Margin = Thickness(5)
			OKbtn.Click += self.OK_Clicked
			self.ButtomSPanel.Children.Add(OKbtn)
	def Yes_Clicked(self,sender,event):
		self.YNBool=1
		self.DialogResult=True
	def No_Clicked(self,sender,event):
		self.YNBool=2
		self.DialogResult=True
...
class PlanSettingForm(Window):
	def __init__(self,PlanJson):
		wpf.LoadComponent(self, r'F:\Proton\RayStation\Script\create file\PlanSetting.xaml')
		#self.Topmost = True
		# Start up window at the center of the screen.
		self.WindowStartupLocation = WindowStartupLocation.CenterScreen
		self.pj = PlanJson
		for part in self.pj.keys():
			self.PartCombo.Items.Add(part)
	...
	def ProtocolCombo_Changed(self,sender,event):
		...
		if not self.ProtocolCombo.SelectedItem == None:
			grid_list=[]
			for beamset in self.pj[self.PartCombo.SelectedItem]['PlanSetting']['PlanProtocol'][self.ProtocolCombo.SelectedItem]['BeamSet']:
	...
	def PartCombo_Changed(self,sender,event):
		if not self.PartCombo.SelectedItem == None:
			self.ProtocolCombo.Items.Clear()
			for protocol in self.pj[self.PartCombo.SelectedItem]['PlanSetting']['PlanProtocol'].keys():
				self.ProtocolCombo.Items.Add(protocol)
			self.GridX.Text = str(self.pj[self.PartCombo.SelectedItem]['PlanSetting']['DoseGrid']['x'])
			self.GridY.Text = str(self.pj[self.PartCombo.SelectedItem]['PlanSetting']['DoseGrid']['y'])
			self.GridZ.Text = str(self.pj[self.PartCombo.SelectedItem]['PlanSetting']['DoseGrid']['z'])
...
if __name__ == '__main__':
	...
	testMsg = MessageBox('Caption','YesNo','Test Message')
	testMsg.ShowDialog()
